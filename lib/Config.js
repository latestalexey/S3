...
class Config {
    ...
    _getConfig() {
        ...
        if (config.port !== undefined) {
            assert(Number.isInteger(this.port) && this.port > 0,
                'bad config: port must be a positive integer');
            this.port = config.port;
        }
        this.ipv6Port = 8002;
        if (config.ipv6Port !== undefined) {
            assert(Number.isInteger(this.ipv6Port) && this.ipv6Port > 0,
                'bad config: ipv6Port must be a positive integer');
            this.ipv6Port = config.ipv6Port;
        }
