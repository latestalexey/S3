...
utils.getBucketNameFromHost = function getBucketNameFromHost(request) {
    ...
    const sliceLength = headers.host.lastIndexOf(':') > 0 ?
        headers.host.lastIndexOf(':') : headers.host.length;
    const host = headers.host.slice(0, sliceLength);

    // parseIp returns empty object if host is not valid IP
    // If host is an IP address, it's path-style
    if (ipCheck.parseIp(host) !== '{}') {
        return undefined;
    }
