 ...
 * @param {AuthInfo} metricObj.authInfo - Instance of AuthInfo class with
 * requester's info
...
export function pushMetric(action, log, metricObj) {
    const data = Object.assign({}, metricObj);
    data.account = metricObj.authInfo.getCanonicalID();
    delete data.authInfo;
    return utapi.pushMetric(action, log.getSerializedUids(), data);
}
