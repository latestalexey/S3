...
function removeTransientOrDeletedLabel(bucket, authInfo, log, callback) {
...
function freshStartCreateBucket(bucket, authInfo, log, callback) {
    const bucketName = bucket.getName();
    metadata.createBucket(bucketName, bucket, log, err => {
        ...
        return addToUsersBucket(authInfo.getCanonicalID(), bucketName, log,
            err => {
                if (err) {
                    return callback(err);
                }
                return removeTransientOrDeletedLabel(bucket, authInfo, log,
                    callback);
            });
    });
}
 ...
 * @return {undefined}
 */
export function cleanUpBucket(bucketMD, authInfo, log, callback) {
    const bucketName = bucketMD.getName();
    return addToUsersBucket(authInfo.getCanonicalID(), bucketName, log, err => {
        if (err) {
            return callback(err);
        }
        return removeTransientOrDeletedLabel(bucketMD, authInfo, log,
            callback);
    });
}
...
export function createBucket(authInfo, bucketName, headers,
    ...
    (err, results) => {
        ...
        if (existingBucketMD === 'NoBucketYet') {
            return bucketLevelServerSideEncryption(
                ...
                (err, sseInfo) => {
                    ...
                    return freshStartCreateBucket(newBucketMD, authInfo,
                                                  log, cb);
                });
        }
        if (existingBucketMD.hasTransientFlag() ||
            ...
            return cleanUpBucket(newBucketMD, authInfo, log, cb);
        }
